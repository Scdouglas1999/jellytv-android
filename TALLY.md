...
| W33 | `ui/playback/PlaybackViewModel.kt` (`init` W29 region; `changeStreams` bitrate), `PlaybackDialog.kt` (W20 region) | in-player Quality: `TallyQuality` override before the global max bitrate, restart at the current position, no video stream copy while a quality is chosen (`transcodingUrl`, which also gets the source: the device profile's `MaxFramerate` is left out when the source is not faster, since Jellyfin 10.10 halves a 60 fps request's bitrate when it picks the size and a 1080p60 channel's "720p · 5 Mbps" played at 960x540); "Quality" menu entry |
...
| W61 | `data/ServerRepository.kt` (after `closeSession`) | `tallyRestoreOffline`: the saved session without asking the server (offline start); `tallyCompleteOfflineSession`: once the server answers again after an offline start, the session is completed as a sign-in completes it (`changeUser`: user details, server info, the saved user, so home rows and libraries reload); upstream's `updateUserDto` only replaces details it already has, so they stayed unknown |
...
| W70 | `ui/CoilConfig.kt` (`components`) | `TallyImageInterceptors` (`media/kit/`): the plugin's game art is asked for at its drawn width and in the device's time zone (`w`, `tz`); failed picture loads are tried again three times with a growing pause |
| W71 | `ui/detail/PlaylistDetails.kt` (`PlaylistViewModel.onMoveItem`) | `PlaylistMove` (`media/playlist/`): on Jellyfin 10.10, whose Move lands one place off, an entry is moved one place up or down with calls that land right; other servers as upstream |
